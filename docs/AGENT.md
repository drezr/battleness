...
- Production forge recipes consume exactly three material units from the matching crafting family. Repeated materials are aggregated into one ingredient quantity. Ingredient rarity pattern is common/common/common for common outputs, refined/common/common for refined outputs, rare/refined/common for rare outputs, and epic/rare/refined for epic outputs. Every active ring, gem, monster, and spell has one stable recipe with a unique material multiset within its item type. Crafted items start at level 1 and quality 0; crafted rings start with one socket.
- Item stat scaling is conservative: item level adds +2% per level after level 1, quality adds a linear bonus from 0% at quality 0 to +25% at quality 100, and resolved values floor after applying the combined multiplier. Rings and gems scale damage, spells scale direct-damage effect amounts, and monsters scale health and damage. Energy, cooldown, speed, and penalties do not scale. Hero speed sums equipped rings, their socketed gems, and their monster or spell enchantments. Ring energy and cooldown penalties sum all decimal gem and enchantment contributions first, then floor the final total once.
...
- Keep player inventory, crafting, credits, loadouts, progression, and reward settlement exclusively in the Prisma-backed Game App. Its Forge > Enchant and Forge > Socket boundaries remain authoritative.
- Keep `apps/prototype` as an internal Dev Lab for direct battle configuration, two-sided control, deterministic scenarios, all active spell and monster interactions, Battle Record import/export, replay verification, raw engine diagnostics, simulations, balance reports, and content/asset inspection.
- Battle Lab configuration must remain independent from player inventory. It directly edits hero and item levels, qualities, ring socket counts, socketed gem definitions, and spell or monster enchantments.
- Browser persistence in the Dev Lab is limited to named Battle Lab presets. Do not add credits, material stock, crafted instances, player loadouts, item XP, combat rewards, or another simulated player profile.
- Dev Lab combat and imported replays must never grant rewards or mutate persistent player-like state. Result summaries remain technical and may report outcome, turns, actions, damage, rings, spells, and monsters.
