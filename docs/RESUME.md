...
- Inventory item details expose persisted socket and enchantment composition as read-only information. Rings link to Forge > Socket, while gems, spells, and monsters link to the dedicated Forge > Enchant view with the relevant item preselected. Enchant permits independent gem and target selection, validates compatibility server-side, and atomically replaces an existing enchantment after confirmation. Replacement and removal are free, preserve both items, allow all elemental combinations, and apply to equipped rings outside combat.
