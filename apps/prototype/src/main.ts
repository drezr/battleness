...
if (!app) {
  throw new Error("Dev Lab app root was not found.");
}
...
clearObsoletePlayerState();
render();

function clearObsoletePlayerState(storage: Storage = localStorage): void {
  for (const version of ["v1", "v2", "v3"] as const) {
    storage.removeItem(`battleness.developmentInventory.${version}`);
    storage.removeItem(`battleness.developmentLoadouts.${version}`);
  }
}
    ...
    <section class="shell">
      ...
      <main class="setup-layout">
        <section class="panel setup-summary">
          ...
          <p class="muted">${escapeHtml(t("ui.devLabPurpose"))}</p>
  ...
  return `
    <section class="battle-lab-tools">
      ...
      <label class="battle-lab-json">
        <span>${escapeHtml(t("ui.battleLabConfigJson"))}</span>
        <textarea
          id="battleLabJson"
          placeholder="${escapeHtml(t("ui.battleLabConfigJsonPlaceholder"))}"
          spellcheck="false"
        >${escapeHtml(battleLabJsonText)}</textarea>
      </label>
      <div class="battle-lab-json-actions">
        <button id="exportBattleLab">${escapeHtml(t("ui.exportBattleLabConfig"))}</button>
        <button id="importBattleLab">${escapeHtml(t("ui.importBattleLabConfig"))}</button>
      </div>
      ${
        battleLabFeedback
    ...
    <article class="lab-ring">
      ...
      <div class="lab-stat-fields">
        ...
        ${renderLabNumberInput(
          ...
          1,
          3,
          `data-lab-player-index="${playerIndex}" data-lab-ring-index="${ringIndex}" data-lab-ring-field="socketCount"`,
        )}
      </div>
      <div class="lab-gems">
        ${ring.gems
...
function bindEvents(): void {
  ...
  document.querySelector<HTMLButtonElement>("#exportBattleLab")?.addEventListener("click", () => {
    battleLabJsonText = serializeBattleLabConfig(battleLabConfig);
    battleLabFeedback = { type: "success", message: t("ui.battleLabConfigExported") };
    render();
  });
  ...
  document.querySelector<HTMLButtonElement>("#importBattleLab")?.addEventListener("click", () => {
    try {
      ...
      battleLabJsonText = serializeBattleLabConfig(battleLabConfig);
      battleLabFeedback = { type: "success", message: t("ui.battleLabConfigImported") };
      state = createState();
    } catch (error) {
      battleLabFeedback = {
        type: "error",
        message: formatMessage("ui.battleLabConfigImportFailed", {
  ...
  document
    ...
    .forEach((control) => {
      control.addEventListener("change", () => {
        ...
        } else if (field === "socketCount") {
          ring.socketCount = clampInteger(control.value, 1, 3);
          ring.gems.splice(ring.socketCount);
        }
        commitBattleLabChange();
      });
    });
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-lab-add-gem]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (ring && ring.gems.length < (ring.socketCount ?? 3)) {
